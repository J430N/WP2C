...
class Password():
    ...
    def run(cls):
        ...
        # Get policy counts
        policies = cls.get_policy_counts(password)
        ...
        cls.print_policy_counts(policies)

        # Calculate password entropy
        entropy = cls.get_password_entropy(policies, entropies)

        # Print password entropy
        print("\n[+] %-25s %d" % ("Password entropy:", entropy))

        # Calculate and print estimated cracking time
        crack_time, time_, strength_level = cls.get_cracking_time(entropy, pass_len, crack_speed)
        cls.print_cracking_time(crack_time, time_, strength_level)
    ...
    def passwd_gen_chk(password):
        ...
        # Get policy counts
        policies = Password.get_policy_counts(password)

        # Remove password from memory
        del password

        # Calculate password entropy
        entropy = Password.get_password_entropy(policies, entropies)

        # Print password entropy
        print("[+] %-25s %d" % ("Password entropy:", entropy))

        # Calculate and print estimated cracking time
        crack_time, time_, strength_level = Password.get_cracking_time(entropy, pass_len, crack_speed)
